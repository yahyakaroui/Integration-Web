# 🍽️ Esprit Meal

## 👥 Nom du groupe
**TWINOVA**

---

## 👤 Membres du groupe

| Nom & Prénom | Profil GitHub |
|--------------|---------------|
| **Yahya Karoui** | https://github.com/yahyakaroui |
| **Malek Fridhi** | https://github.com/Malek-Fridhi |
| **Khalil Triki** | https://github.com/khaliltriki02 |
| **Nour Bouslimi** | https://github.com/Nour-Bouslimi |
| **Nasef Fadhlaoui** | https://github.com/fadhlaouinassef |

---

## 🖼️ Captures des interfaces par membre

> Cette section présente les interfaces développées par chaque membre de l’équipe dans l’application **Esprit Meal**.

### 🔹 Interface – Yahya Karoui
![Interface Yahya](public/1.png)
![Interface Yahya](public/2.png)
![Interface Yahya](public/3.png)
![Interface Yahya](public/4.png)
![Interface Yahya](public/5.png)
![Interface Yahya](public/6.png)

---

### 🔹 Interface – Malek Fridhi
![Interface Malek](screenshots/malek_interface.png)

---

### 🔹 Interface – Khalil Triki
![Interface Khalil](screenshots/khalil_interface.png)

---

### 🔹 Interface – Nour Bouslimi
![Interface Nour](screenshots/nour_interface.png)

---

### 🔹 Interface – Nasef Fadhlaoui
![Interface Nasef](screenshots/nasef_interface.png)

---

## 📱 Détails du projet

### 🔸 Nom de l’application
**Esprit Meal**

### 🔸 Description
**Esprit Meal** est une application web dédiée à la gestion et à la commande de repas.  
Elle permet aux utilisateurs de consulter des menus, passer des commandes et visualiser leurs choix à travers une interface moderne et intuitive.

Ce projet a été réalisé dans un cadre académique en respectant les bonnes pratiques du développement web et du travail en équipe.

---

## 🛠️ Technologies utilisées

- HTML5  
- CSS3  
- Tailwind CSS  
- JavaScript  
- Git & GitHub  
- Font Awesome  

---

## 📂 Dossier des captures


